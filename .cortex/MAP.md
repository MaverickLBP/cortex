...
- `SYSTEM.md` → **Agent behaviour instructions.** Do not edit manually. Defines how the agent should load, use and maintain MAP.md. Version 4.2.0.
...
- `settings.json` → **Committed** Claude Code settings. Registers the `cortex-session.sh` SessionStart hook, the `cortex-file-change.sh` PostToolUse hook, the `cortex-subagent.sh` SubagentStart hook, and the `cortex-map-load.sh` PreToolUse hook. Per-user overrides go in `.claude/settings.local.json` (gitignored).
...
- `cortex-map-load.sh` → **PreToolUse hook template.** Safety net for hierarchical (large) projects. On `Grep`/`Glob`, reminds the agent to consult `.cortex/maps/index.json` and read the resolved sub-map instead of searching blind. On `Write` to a new path, reminds the agent to place the file per MAP.md's documented conventions and keep the relevant sub-map in sync. Silent on flat projects (no `.cortex/maps/` directory) and on non-CORTEX paths.
...
**2026-07-23 — v4.2.0 hierarchical map: scan/partition scripts + PreToolUse safety net**
CORTEX itself stays a small, flat project (no `.cortex/maps/` here), but the machinery for large projects was added this release: `cortex-scan.sh` (gitignore-aware file/dir enumeration), `cortex-areas.sh` (scale-adaptive area partition + `index.json` manifest writer), and the `cortex-map-load.sh` PreToolUse hook (Grep/Glob discovery reminder + Write placement reminder, silent on flat projects like this one). `SYSTEM.md` §2.5 carries the full standing-instruction mechanism agent-agnostically — it's the entire mechanism on OpenCode, since OpenCode has no hooks. Covered by `tests/map-test.sh`.
