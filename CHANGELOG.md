...
- **Knowledge moved to `.cortex/`** — all files previously in `.claude/cortex/` are now in `.cortex/` (agent-agnostic). Run `install.sh` to migrate existing projects automatically (MAP.md is preserved).
...
- Automatic migration in `install.sh`: detects old `.claude/cortex/` layout and moves knowledge to `.cortex/`
